...
class CoordinadorController
{
    ...
    {
        try {
            // Validar que el cambioId existe y es válido
    ...
    public function getProgresoEvaluacionFinal()
    {
        try {
            ...
            // Consultar estado de cierre de fase final
            $sqlCierreFaseFinal = "
            ...
            // -----------------------------------------------
            // Datos específicos de fase final (medallas, top, configuración)
            // -----------------------------------------------
            $medallasAsignadas = [
                'oro' => 0,
                'plata' => 0,
            ...
            if ($faseFinalCerrada) {
                // Contar medallas usando medalla_asignada
                $sqlMedallas = "
                    SELECT 
                ...
                // Top 5 clasificados por nota promedio (incluye su medalla)
                $sqlTop = "
                    SELECT 
                        o.nombre_completo,
                ...
                // Obtener configuración de medallero (general o por nivel/grado)
                try {
                    // Primero intentar obtener configuración general (nivel_competencia_id IS NULL)
                    $sqlCheckConfig = "
                    ...
                    // Si no hay configuración general, buscar cualquier configuración para el área
            ...
            $responseData = [
                ...
                // Datos de fase final para el dashboard de cierre
...
public function registrarTiemposEvaluadores()
{
    ...
    public function getDashboardCierreFaseArea()
    {
        try {
            ...
            if ($totalParticipantes > 0) {
                // Verificar si la fase está cerrada antes de actualizar
                $estaCerrada = $cierre && ($cierre['estado'] === 'cerrada' || $cierre['fecha_cierre'] !== null);
                
                if (!$cierre) {
                    // Solo crear registro si no existe y no está cerrada
                    $sqlInsert = "
                    ...
                    // Solo actualizar si NO está cerrada
                    $sqlUpdate = "
                    ...
                    // Si está cerrada, recargar desde BD para asegurar que tenemos el estado correcto
                    $stmtCierre->execute([$areaId]);
                    $cierre = $stmtCierre->fetch(PDO::FETCH_ASSOC);
        ...
        try {
            ...
            if (!empty($todosEvaluados)) {
                ...
                foreach ($todosEvaluados as $participante) {
                    ...
                    // Mostrar grado de escolaridad si existe, si no mostrar nivel
    ...
    public function descargarReportePDFCierreFase()
    {
        try {
            ...
            if (empty($files) || $necesitaRegenerar) {
                // Obtener datos del cierre de fase final
                $sqlCierre = "
                    SELECT estado, fecha_cierre
                    FROM cierre_fase_areas
                    WHERE area_competencia_id = ? 
                ...
                // Obtener configuración de olimpiada para puntuación mínima y porcentaje
                $puntuacionMinima = 51.00;
                $porcentajeClasificados = 0.6;
                ...
                try {
                    ...
                    // Si hay PDF existente, continuar con ese
                }
            }
            ...
            // Limpiar cualquier salida previa antes de enviar error
            while (ob_get_level()) {
                ob_end_clean();
            }
    ...
    /**
     * Cerrar fase final del área y calcular medallas
     */
    public function cerrarFaseFinal()
    {
        try {
            ...
            // Verificar que la fase clasificatoria esté cerrada
            ...
            // Obtener estadísticas de participantes clasificados
            $sqlStats = "
                SELECT 
            ...
            // Verificar que existe configuración de medallero
            // Primero buscar configuración general (nivel_competencia_id = NULL)
            $medalleroModel = new ConfiguracionMedallero();
            $configMedallero = $medalleroModel->getByAreaAndLevel($areaId, null);
            
            // Si no hay configuración general, buscar configuraciones por nivel
            if (!$configMedallero) {
                $sqlConfigNiveles = "
                    SELECT COUNT(*) as total
                    FROM configuracion_medallero
                ...
                // Si hay configuraciones por nivel, usar la primera como referencia
                // (el cálculo de medallas manejará cada nivel por separado)
                $sqlPrimeraConfig = "
                    SELECT *
                    FROM configuracion_medallero
                    WHERE area_competencia_id = ?
            ...
            // Verificar si la fase final ya está cerrada
            // Verificamos si hay participantes con medallas asignadas (usando medalla_asignada)
            ...
            // Si ya hay medallas asignadas y hay un registro de cierre reciente, asumimos que ya está cerrada
            if ($verificarMedallas && (int)$verificarMedallas['total_con_medalla'] > 0) {
                // Verificar si hay un cierre reciente de fase final (últimas 24 horas)
                $sqlCierreReciente = "
            ...
            // Calcular y asignar medallas
            $medallasAsignadas = $this->calcularMedallasFaseFinal($areaId, $configMedallero);
            
            // Actualizar estado de cierre de fase final
            // Usamos fase = 'final' para distinguir del cierre de fase clasificatoria
            $sqlUpdate = "
                INSERT INTO cierre_fase_areas (
                    area_competencia_id, nivel_competencia_id, fase, estado,
                    porcentaje_completitud, cantidad_clasificados, 
            ...
            // Finalizar respuesta para continuar en segundo plano
            if (function_exists('fastcgi_finish_request')) {
                fastcgi_finish_request();
            } else {
                if (ob_get_level()) {
    ...
    /**
     * Calcular medallas para fase final según configuración
     */
    private function calcularMedallasFaseFinal($areaId, $configMedallero)
    {
        ...
        // Obtener todos los participantes clasificados con sus puntuaciones finales
        $sql = "
            SELECT 
        ...
        // Agrupar por grado escolaridad para aplicar configuración específica
        ...
        // Procesar cada grupo de grado
        foreach ($participantesPorGrado as $grado => $participantesGrado) {
            // Obtener configuración específica por grado si existe
            ...
            foreach ($participantesGrado as $participante) {
                ...
                // Asignar medalla según rangos y límites de la configuración de medallero
                // Prioridad: Oro > Plata > Bronce > Mención de Honor
                // Se respetan los rangos de puntuación y las cantidades máximas configuradas
                if ($contadorOro < $oroMax && $puntuacion >= $oroMin && $puntuacion <= $oroMaxPunt) {
                    ...
                    // Mención de honor se asigna a los siguientes mejores puntajes que no calificaron para medallas
                    $medalla = 'mencion_honor';
                    $contadorMencion++;
                } else {
                    $medalla = null; // Sin medalla - no cumple con ningún criterio de la configuración
                }

                // Actualizar medalla_asignada en inscripciones_areas
                // Mantenemos el estado como 'clasificado' y usamos medalla_asignada para el tipo de medalla
                $sqlUpdate = "
    ...
    /**
     * Obtener datos necesarios para regenerar reportes de fase final
     */
    private function obtenerDatosFaseFinal($areaId)
    {
        error_log("obtenerDatosFaseFinal: Iniciando para área {$areaId}");
        
        // Primero verificar si hay un registro de cierre de fase final
        // Buscar TODOS los registros de cierre para esta área para debug
        $sqlTodosCierres = "
        ...
        // Buscar específicamente el registro de fase final CERRADA
        // Priorizar el que tiene estado 'cerrada' y fecha_cierre no null
        $sqlCierre = "
        ...
        // Si no encontramos uno cerrado, buscar cualquier registro con estado cerrada (sin restricción de fecha)
        if (!$cierre) {
            $sqlCierre2 = "
                SELECT fecha_cierre, estado
                FROM cierre_fase_areas
        ...
        // Verificar si hay medallas asignadas (indicador de que la fase final está cerrada)
        $sqlVerificarMedallas = "
            SELECT COUNT(*) as total_con_medalla
            FROM inscripciones_areas
            WHERE area_competencia_id = ? 
        ...
        // También verificar todos los estados de inscripciones para debug
        $sqlTodosEstados = "
        ...
        // Si no hay registro de cierre ni medallas, la fase final no está cerrada
        if (!$cierre && !$tieneMedallas) {
            error_log("obtenerDatosFaseFinal: No hay registro de cierre ni medallas, retornando null");
            return null;
        }
        
        // Si hay medallas pero no registro de cierre, usar fecha actual
        if (!$cierre && $tieneMedallas) {
        ...
        // Si hay registro de cierre cerrada pero no hay medallas, intentar regenerar las medallas
        ...
        // Después de cerrar la fase final, los participantes pueden tener estados de medalla
        // Por lo tanto, contamos todos los que tienen evaluaciones finales como clasificados
        $sqlStats = "
            SELECT 
                COUNT(DISTINCT ia.id) FILTER (
        ...
        // Contar medallas asignadas (usando medalla_asignada)
        $sqlMedallas = "
            SELECT 
                COUNT(*) FILTER (WHERE medalla_asignada = 'oro') as oro,
                COUNT(*) FILTER (WHERE medalla_asignada = 'plata') as plata,
    ...
    /**
     * Generar reporte PDF de cierre de fase final
     */
    private function generarReportePDFCierreFaseFinal($areaId, $areaNombre, $totalClasificados, $totalEvaluados, $medallasAsignadas, $fechaCierre, $coordinador)
    {
        try {
            ...
            $pdf->Cell(0, 6, $medallasAsignadas['mencion_honor'] ?? 0, 0, 1, 'L');
            $pdf->Cell(100, 6, 'Sin medalla:', 0, 0, 'L');
            $pdf->Cell(0, 6, $medallasAsignadas['sin_medalla'] ?? 0, 0, 1, 'L');
            $pdf->Ln(5);
            ...
            try {
                ...
                // Obtener configuración general del área
                $configMedallero = $medalleroModel->getByAreaAndLevel($areaId, null);
                
                // Obtener configuraciones específicas por grado si existen
                $sqlConfigGrados = "
                ...
                if ($configMedallero) {
                    ...
                    // Mostrar configuraciones específicas por grado si existen
            ...
            // Obtener participantes por medalla (usando medalla_asignada)
            ...
            foreach ($tiposMedalla as $tipoMedalla) {
                ...
                if (!empty($participantes)) {
                    $pdf->AddPage();
                    $pdf->SetFont('helvetica', 'B', 14);
                    $pdf->Cell(0, 8, 'MEDALLA DE ' . strtoupper($tipoMedalla), 0, 1, 'C');
                    $pdf->Ln(3);
            ...
            // Lista completa de TODOS los participantes (premiados y no premiados)
            // Incluye todos los que tienen evaluaciones finales, independientemente de su medalla
            $sqlTodos = "
                SELECT 
                    o.nombre_completo,
                    o.documento_identidad,
    ...
    /**
     * Generar reporte Excel de participantes fase final
     */
    private function generarReporteExcelParticipantesFaseFinal($areaId, $areaNombre, $medallasAsignadas, $fechaCierre, $coordinador)
    {
        try {
            ...
            $sheet->setCellValue('B9', $medallasAsignadas['mencion_honor'] ?? 0);
            $sheet->setCellValue('A10', 'Sin Medalla:');
            $sheet->setCellValue('B10', $medallasAsignadas['sin_medalla'] ?? 0);
            ...
            $row = 2;
            foreach ($todosParticipantes as $participante) {
                $medallaTexto = $participante['medalla'] ? ucfirst(str_replace('_', ' ', $participante['medalla'])) : 'Sin medalla';
    ...
    private function generarReportePDFEstadisticasFaseFinal($areaId, $areaNombre, $medallasAsignadas, $fechaCierre, $coordinador)
    {
        try {
            ...
            $pdf->Cell(90, 6, $medallasAsignadas['mencion_honor'] ?? 0, 1, 1, 'C');
            $pdf->Cell(100, 6, 'Sin Medalla', 1, 0, 'L');
            $pdf->Cell(90, 6, $medallasAsignadas['sin_medalla'] ?? 0, 1, 1, 'C');
    ...
    public function descargarReportePDFCierreFaseFinal()
    {
        try {
            ...
            // Si no existe el archivo, verificar si la fase está cerrada y regenerar
            if (empty($files)) {
    ...
    public function getParticipantesPremiados()
    {
        try {
            ...
            // Incluir tanto los que tienen medalla (oro, plata, bronce) como los sin medalla (para certificados de participación)
            $sqlParticipantes = "
                SELECT 
                    ia.id,
    ...
    /**
     * Aprobar certificados del área del coordinador
     */
