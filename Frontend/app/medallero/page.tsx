...
export default function PublicResults() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await PublicResultsService.getMedalleroFinal()
        setOlimpistas((data as unknown as { olimpistas: Olimpista[] }).olimpistas || [])
        setConfigPublicacion((data as any).configPublicacion || [])
      } catch (err) {
...
const today = new Date();
  ...
  const filteredResults = useMemo(() => {
    return olimpistas.filter((o) => {
      ...
      const withinDate =
      areaConfig &&
      today >= new Date(areaConfig.publicacion_inicio) 
