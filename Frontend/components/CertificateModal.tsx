...
export default function CertificateModal({
  ...
  // Solo mostrar áreas aprobadas en el modal
  ...
  // Solo de áreas aprobadas
  useEffect(() => {
    const initial: Record<number, boolean> = {};
    // compute per area - solo áreas aprobadas
  ...
  }, [approvedAreas, open]); // re-initialize when areas change or modal opens
  ...
  function computeParticipationIds(area: Area, honorIds: number[]) {
    // Participación: participantes SIN medalla asignada (medalla_asignada IS NULL)
    // Los de mención de honor son los que NO tienen medalla asignada, por eso se les da certificado de participación
    return area.competitors
      .filter(c => {
        // Excluir los que ya están en honor
        ...
        // Incluir participantes SIN medalla asignada (estado es null, undefined, o vacío)
        // Estos son los de mención de honor que reciben certificado de participación
        return !estado || estado === null || estado === '' || estado === 'null' || estado === 'undefined';
      })
      .map(c => c.id);
  }

  // candidatesByArea: produce honor y participación para TODAS las áreas (aprobadas o no)
